...
class StrokeViewController: ExampleBaseViewController {
    ...
    private func makeText() -> NSAttributedString {
        NSAttributedString(style: PoTextStyle(font: .systemFont(ofSize: 30))) {
            PoText("Typography StrokeWidth -4").stroke(width: -4, color: .red)
            padding
            PoText("Typography StrokeWidth 0").stroke(width: 0, color: .orange)
            padding
            PoText("Typography StrokeWidth 4").stroke(width: 4, color: .green)
        }
    }
